        ...
        <div class="d-inline-flex">
            ...
            <form class="form-inline" action="{{ route('booksList') }}" method="GET">
                <div class="input-group">
                    <input class="form-control mr-sm-2" type="search" placeholder="Название книги" name="search" value="{{ request()->input('search') ? request()->input('search') : ''}}">
            ...
            <nav aria-label="...">
                <ul class="pagination pagination-sm">
                    <li class="page-item @if($books->currentpage() == 1) disabled @endif">
                        <a class="page-link" href="{{$books->previousPageUrl()}}{{request()->has('search') ? '&search=' . request()->get('search') : ''}}" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    @for ($i = 1; $i <= $books->lastPage(); $i++)
                        <li class="page-item @if ($books->currentPage() == $i) disabled @endif">
                            <a class="page-link" href="{{$books->resolveCurrentPath()}}?page={{$i}}{{ request()->has('search') ? '&search=' . request()->get('search') : ''}}" @if ($books->currentPage() == $i) tabindex="-1" @endif
                            >{{$i}}</a>
                        </li>
                    @endfor
                    <li class="page-item @if($books->currentpage() == $books->lastPage()) disabled @endif">
                        <a class="page-link" href="{{$books->nextPageUrl()}}{{request()->has('search') ? '&search=' . request()->get('search') : ''}}" aria-label="Next">
